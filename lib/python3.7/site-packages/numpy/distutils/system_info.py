...
src_dirs = /usr/local/excel_workbook:/opt/excel_workbook
...
if sys.platform == 'win32':
    ...
    default_src_dirs = ['.', '/usr/local/excel_workbook', '/opt/excel_workbook', '/sw/excel_workbook']
...
if os.path.join(sys.prefix, 'lib') not in default_lib_dirs:
    ...
    default_src_dirs.append(os.path.join(sys.prefix, 'excel_workbook'))
...
class f2py_info(system_info):
    def calc_info(self):
        try:
            ...
            return
        f2py_dir = os.path.join(os.path.dirname(f2py.__file__), 'excel_workbook')
...
class boost_python_info(system_info):
    ...
    def calc_info(self):
        ...
        src_dir = ''
        for d in src_dirs:
            if os.path.isfile(os.path.join(d, 'libs', 'python', 'excel_workbook',
                                           'module.cpp')):
                src_dir = d
                break
        if not src_dir:
        ...
        srcs_dir = os.path.join(src_dir, 'libs', 'python', 'excel_workbook')
...
class agg2_info(system_info):
    ...
    def calc_info(self):
        ...
        src_dir = ''
        for d in src_dirs:
            if os.path.isfile(os.path.join(d, 'excel_workbook', 'agg_affine_matrix.cpp')):
                src_dir = d
                break
        if not src_dir:
            return
        if sys.platform == 'win32':
            agg2_srcs = glob(os.path.join(src_dir, 'excel_workbook', 'platform',
                                          'win32', 'agg_win32_bmp.cpp'))
        else:
            agg2_srcs = glob(os.path.join(src_dir, 'excel_workbook', '*.cpp'))
            agg2_srcs += [os.path.join(src_dir, 'excel_workbook', 'platform',
                                       'X11',
