...
def plot_by_eq(gas_list: list, instance_list: [Workbook], final_part_title: str, colour_list: list= None, legend_list:list=None):
    for gas in gas_list:
        ...
        for instance, colour in zip(instance_list, colour_list):
            ...
            eq_workbooks.round_col('mean_eq')
            eq_workbooks.round_col('mean_heat')

            # assign these three dfs to a dictionary containing x and y values:
            d = graph_funcs.assign_xy_from_list('mean_heat', 'error_heat', 'X_' + gas, 'delta_X_' + gas, eq_workbooks.df_list, 'mean_eq')
