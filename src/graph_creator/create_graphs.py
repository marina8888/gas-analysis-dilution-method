...
def plot_by_workbook(gas_list: list, instance_list: [Workbook], final_part_title: str, colour_list: list=None, legend_list:list=None):
    ...
    for gas in gas_list:
        fig = plt.figure(figsize=(6.5, 6))
        #title artguments - first part+final part can be from list or variable (e.g gas, eq_ratio)
        graph_funcs.format_graph(gas, final_part_title, fig, 'N2 fLowrate, (SLM)', gas+'concentration, (ppmv)')
        ...
        for instance, colour in zip(instance_list, colour_list):
            ...
            d = graph_funcs.assign_xy_from_inst('N2 upper', 'Qd1_upper', 'X_' + gas, 'delta_X_' + gas, instance)

            # plot error bars based on dictionary values for all three dataframes
            graph_funcs.plot_error(d, fig)
            graph_funcs.polyfit_xy(d, fig)
            # plot scatter graph with legends (default legends and colours available here)
            graph_funcs.plot_scatter(d, fig, colour, gas)
        plt.savefig('../excel_external/image_plots/' + 'CO2Conc' + gas)

def plot_all_gas_on_one(gas_list: list, instance_list: [Workbook], final_part_title: str, colour_list: list= None):
    if len(colour_list) != len(gas_list):
        raise ValueError("pls input same number of colours in colour_list as legends in the legend_list")

    fig = plt.figure(figsize=(6.5, 6))
    #title artguments - first part+final part can be from list or variable (e.g gas, eq_ratio)
    graph_funcs.format_graph("", final_part_title, fig, 'N2 fLowrate, (SLM)', 'gas deviation from mean, (ppmv)')
    for instance in instance_list:
        #iterate over all instances (heat fractions) on one gas plot:
        for colour, g in zip(colour_list, gas_list):
            #assign x and y values to a dictionary to use as graph data:
            d = graph_funcs.assign_xy_from_inst('N2 upper', 'Qd1_upper', 'X_' + g, 'delta_X_' + g, instance)

            # plot error bars based on dictionary values for all three dataframes
            # graph_funcs.plot_error(d, fig, colour)
            graph_funcs.polyfit_xy(d, fig, colour)
            # plot scatter graph with legends (default legends and colours available here)
            graph_funcs.plot_scatter(d, fig, colour, g)
        plt.savefig('../excel_external/image_plots/' + 'allN2')
